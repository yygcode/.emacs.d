...
** basic keybind
Turn off minor mode y/basic-keybind-global-mode if you don't like.
#+begin_src emacs-lisp
  (require 'y-keybinds)
  (require 'y-browse)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (define-key
    y/basic-keybind-mode-map (kbd "C-x C-x l")
...
#+begin_src emacs-lisp
  ...
  (defhydra y/window-hydra
    (y/basic-keybind-mode-map "C-x C-x w")
    "window hydra"
...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    (projectile-mode 1)
    :bind
    ("C-x C-x p" . projectile-command-map)
    ("C-x C-x C-p" . projectile-command-map))
